...
class ParserController extends Controller
{
    ...
    {
        $step = 86400;
        $tstop = strtotime(date('d.m.Y', time())) + $step - 1;
        $tstart = $tstop - 6 * $step + 1;
        ...
        $siteNames = $command->selectDistinct('name')
            ->from('parser')
    ...
    {
        $model = new Parser('search');
        $model->unsetAttributes(); // clear any default values
        if (isset($_GET['Parser']))
            $model->attributes = $_GET['Parser'];
     ...
     * @param Parser $model the model to be validated
     */
